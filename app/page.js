...
export default function page() {
  ...
  return (
    <div className='h-dvh md:min-h-screen xl:min-h-screen w-screen bg-gray-950 text-white flex flex-col items-center overflow-hidden'>
      <h1 className=" mt-6 text-3xl md:mt-8 md:text-4xl xl:mt-10 xl:text-5xl font-bold tracking-wider">4 In A Row</h1>
      {
        winner && <h2 className="text-xl md:text-xl xl:text-2xl font-bold tracking-wide md:mt-10 xl:mt-12 mt-8">Player {winner} wins!</h2>
      }
      {
        isDraw && <h2 className="text-xl md:text-xl xl:text-2xl font-bold tracking-wide md:mt-10 xl:mt-12 mt-8">Match Drawn!</h2>
      }
      <div className="flex-1 mt-6 w-full flex justify-center relative">

        <div className="md:w-112 md:h-87 xl:w-144 xl:h-122 w-72 h-61 self-end mb-2 md:mb-2 xl:mb-2">
          <Board board={board} />
        </div>
        {
        ...
        <button
          onClick={restartGame}
          className="hidden md:hidden xl:block absolute bottom-12 left-8 bg-gray-200 text-red-700 font-bold px-4 py-2 rounded cursor-pointer hover:bg-gray-400">
          Restart Game
        </button>
      ...
      <button onClick={restartGame}
          className="block md:block xl:hidden absolute top-6 left-4 text-red-700 bg-gray-200 p-2 rounded-md cursor-pointer">
            <RotateCcw />
        </button>
    </div>
  )
